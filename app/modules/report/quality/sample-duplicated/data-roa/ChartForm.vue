...
const useDummy = ref(true) // ubah true/false
function generateDummyScatter(analyte: string, count = 650): ScatterSection {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const x = Number(base.toFixed(3))
    const y = Number((base + noise).toFixed(3))
    ...
    points.push({
      ...
      material: "Limonite",
      sample_number: `DUP-${analyte.toUpperCase()}-${i + 1}`,
      sample_original: `ORI-${analyte.toUpperCase()}-${i + 1}`,
      iup_code: "IUP-001",
    })
  }
...
//       "/api/analytics/raw/geology/sample-duplicated-roa/scatter",
//       {
//         method: "GET",
//         query: {
...
async function fetchPlotData() {
  if (useDummy.value) {
    plotData.value = {
      ni: generateDummyScatter("ni"),
      co: generateDummyScatter("co"),
      fe: generateDummyScatter("fe"),
      mgo: generateDummyScatter("mgo"),
      sio2: generateDummyScatter("sio2"),
    }
    return
  }
