...
type ColumnActions = {
  onEdit: (row: SampleDupMralRow) => void
  onDelete: (row: SampleDupMralRow) => void
}
...
export function getSampleDupMralColumns(
  actions: ColumnActions
): ColumnDef<SampleDupMralRow>[] {
  return [
    {
      id: "select",
      header: ({ table }) =>
    ...
    ...analyteColumns.flatMap((col) => {
      ...
      return [
        ...
        },
        {
          ...
          cell: ({ row }: any) =>
            h(
              ...
              formatNumber(
                row.original[`${key}_ori` as keyof SampleDupMralRow] as
                  | number
                  | null
                  | undefined
              )
        ...
        },
        {
          ...
          cell: ({ row }: any) => {
            const value = row.original[
              `${key}_diff` as keyof SampleDupMralRow
        ...
        },
        {
          ...
          cell: ({ row }: any) => {
            const value = row.original[
              `${key}_rel_diff` as keyof SampleDupMralRow
            ] as number | null | undefined
        ...
        },
        {
          ...
          cell: ({ row }: any) => {
            const value = row.original[
              `${key}_rel_abs` as keyof SampleDupMralRow
            ] as number | null | undefined
        ...
        },
        {
          ...
          cell: ({ row }: any) =>
            renderRemark(
              row.original[`${key}_error` as keyof SampleDupMralRow] as
                | number
                | null
                | undefined
            ),
        },
      ] as ColumnDef<SampleDupMralRow>[]
    }),
  ]
}