...
async function fetchChartData() {
  ...
  try {
    const res = await request<ChartResponse>("/api/analytics/raw/geology/chart-tat/roa", {
      method: "GET",
      query: {
    ...
    chartData.value = null
    notify.error(err?.message || "Failed to load TAT ROA chart")
  } finally {
...
async function fetchTableData() {
  ...
  try {
    const res = await request<TableResponse>("/api/analytics/raw/geology/data-tat/roa", {
      method: "GET",
      query: {
        iup_id: plotFilters.value.iup_id ?? undefined,
        startDate: plotFilters.value.start_date,
    ...
    tableData.value = null
    notify.error(err?.message || "Failed to load TAT ROA table")
  } finally {
...
const roaLayout = computed(() => {
  const title =
    plotFilters.value?.start_date && plotFilters.value?.end_date
      ? `TAT ROA (${plotFilters.value.start_date} to ${plotFilters.value.end_date})`
      : "TAT ROA"
...
const summaryTableSection = computed<TableSection>(() => {
  ...
  return {
    ...
    rows: rows.map((row) => ({
      ...
      limit: row.time_limit ?? "192:00:00",
    })),
  }
})
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    ...
    <div v-if="plotLoading || tableLoading" class="rounded-xl border p-6 text-sm text-muted-foreground">
      Loading TAT ROA data...
    </div>

    <div v-else class="grid grid-cols-1 xl:grid-cols-2 gap-4">
      <div class="rounded-xl border p-3 space-y-3">
        <div>
          <h3 class="text-base font-semibold">TAT ROA Summary</h3>
          <p class="text-xs text-muted-foreground">
            Samples, TAT ROA and limit by selected date range
          </p>
        </div>
      ...
      <div class="mt-2">
        <TatRoaSummaryTable title="TAT ROA Summary Table" :section="summaryTableSection" :loading="tableLoading" />
      </div>
    </div>
  </div>
</template>