...
import { inventoryFilters } from "@/modules/inventory/stockpiles/filters"
...
watch(
  ...
  (v) => {
    local.value = {
      ...
      sampling_area: Array.isArray(v?.sampling_area) ? [...v.sampling_area] : [],
...
watch(
  () => [local.value.iup_id, local.value.material],
  ([newIupId, newMaterial], [oldIupId, oldMaterial]) => {
    if (newIupId !== oldIupId || newMaterial !== oldMaterial) {
...
function apply() {
  emit("apply", {
    ...
    sampling_area: Array.isArray(local.value.sampling_area)
      ? [...local.value.sampling_area]
      : [],
...
const activeBadges = computed(() => {
  const items: string[] = []
  const areas = Array.isArray(local.value.sampling_area)
    ? local.value.sampling_area
    : []

  if (local.value.cut_date) items.push(`Cut Date: ${local.value.cut_date}`)
  if (local.value.iup_id) items.push(`IUP: ${local.value.iup_id}`)
  if (local.value.material) items.push(`Material: ${local.value.material}`)
  areas.forEach((v) => items.push(`Stockpile: ${v}`))
