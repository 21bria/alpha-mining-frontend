...
import DomeCloseFormDialog, {
  type DomeClosePayload,
} from "@/modules/geology/dome-status/close/components/CloseDialog.vue"

import {
  getDomeCloseColumns,
  type DomeCloseRow,
} from "@/modules/geology/dome-status/close/columns"

import { domeCloseConfig } from "@/modules/geology/dome-status/close/table"
import { domeCloseFilters } from "@/modules/geology/dome-status/close/filters"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<DomeCloseRow>>(
  () => `close-dome:${JSON.stringify(query.value)}`,
  () => request(domeCloseConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(domeCloseConfig.defaultQuery ?? {}) }
  page.value = 1
  refresh()
}
...
const selected = ref<DomeCloseRow | null>(null)
...
const openEdit = (row: DomeCloseRow) => {
  if (!canMutate.value) return
  formErrors.value = null
  selected.value = row
  mode.value = "edit"
...
const submit = async (payload: DomeClosePayload) => {
  if (!canMutate.value) return
  ...
  try {
    if (mode.value === "create") {
      await request(domeCloseConfig.endpoint, { method: "POST", body: payload })
      notify.success(`Dome status "${payload.status_dome}" created`)
    } else {
      await request(`${domeCloseConfig.endpoint}${payload.id}/`, {
        method: "PATCH",
        body: payload,
      })
...
const selectedDelete = ref<DomeCloseRow | null>(null)

function remove(row: DomeCloseRow) {
  selectedDelete.value = row
  deleteOpen.value = true
}

async function deleteRows(ids: (string | number)[]) {
  return request("/api/geology/close-dome/bulk-delete/", {
    method: "POST",
    body: {
      ids,
      hard: true,
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
      ...
      <div>
        <h3 class="text-lg font-semibold">Dome Close</h3>
        <p class="text-sm text-muted-foreground">Manage dome status (close).</p>
      </div>
    </div>
    <DataTableMaster
      :columns="columns"
      ...
      :search="search"
      :loading="pending"
      :filtersSchema="domeCloseFilters"
      :showImport="false"
      :showExport="false"
