...
import { getSellingColumns, type SellingRow } from "@/modules/geology/samples/selling/columns"
import { samplesConfig, samplesCRUDConfig } from "@/modules/geology/samples/selling/table"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<SellingRow>>(
  () => `samples-selling:${JSON.stringify(query.value)}`,
...
// edit
async function openEdit(row: SellingRow) {
...
async function handleDownloadTemplate() {
  ...
  const data = await request<ArrayBuffer>(
    "/api/selling/samples-crud/download-template/",
    {
      method: "GET",
  ...
  a.href = url
  a.download = "selling_samples_import_template.xlsx"
...
// delete
function remove(row: SellingRow) {
...
<template>
  <div class="space-y-4">
    ...
    <DataTableMaster 
      ...
      @update:search="(v) => { search = v; debouncedSearch() }" @applyFilters="onApply" @resetFilters="onReset"
      @changePage="(v) => { page = v; refresh() }" @changePageSize="(v) => { pageSize = v; page = 1; refresh() }"
    ...
    <ImportDialog 
      ...
      title="Import Samples selling" 
      moduleKey="selling.samples"
      importUrl="/api/selling/samples-crud/import/" 
    ...
    <ExportDialog
      ...
      endpoint="/api/selling/samples-crud/export/"
      method="GET"
      mode="async"
