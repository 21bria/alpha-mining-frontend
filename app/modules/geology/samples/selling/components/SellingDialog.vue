...
const dischargeAreaPage = ref(1)
const dischargeAreaHasMore = ref(true)

const productCodeOptions = ref<LookupOption[]>([])
const productCodeLoading = ref(false)
const productCodeSearch = ref("")
const productCodePage = ref(1)
const productCodeHasMore = ref(true)
 ...
 * FETCH: Discherge
 * ========================= */
async function fetchDischargeAreas(q = "", page = 1) {
  if (dischargeAreaLoading.value) return
  ...
  try {
    ...
    const count = Number(res?.count ?? 0)
    assignPagedOptions(dischargeAreaOptions, items, page)
    dischargeAreaPage.value = page
    dischargeAreaHasMore.value = computeHasMore(dischargeAreaOptions.value.length, count)
  } finally {
    dischargeAreaLoading.value = false
  }
}

const ondischargeAreaSearch = useDebounceFn((q: string) => {
  dischargeAreaPage.value = 1
  dischargeAreaHasMore.value = true
  fetchDischargeAreas(q, 1)
}, 300)

function ondischargeAreaScroll(e: Event) {
  const el = e.target as HTMLElement
  const nearBottom = el.scrollTop + el.clientHeight >= el.scrollHeight - 40
  if (nearBottom && dischargeAreaHasMore.value && !dischargeAreaLoading.value) {
    fetchDischargeAreas(dischargeAreaSearch.value, dischargeAreaPage.value + 1)
  }
}
 ...
 * FETCH: Product Code
 * ========================= */
async function fetchProductCodes(q = "", page = 1) {
  if (productCodeLoading.value) return
  ...
  try {
    ...
    const items: LookupOption[] = (res?.results ?? []).map((item: any) =>
      toLookupOption(item, ["id"], ["code", "name"])
    )

    const count = Number(res?.count ?? 0)
    assignPagedOptions(productCodeOptions, items, page)
    productCodePage.value = page
    productCodeHasMore.value = computeHasMore(productCodeOptions.value.length, count)
  } finally {
    productCodeLoading.value = false
  }
}

const onproductCodeSearch = useDebounceFn((q: string) => {
  productCodePage.value = 1
  productCodeHasMore.value = true
  fetchProductCodes(q, 1)
}, 300)

function onproductCodeScroll(e: Event) {
  const el = e.target as HTMLElement
  const nearBottom = el.scrollTop + el.clientHeight >= el.scrollHeight - 40
  if (nearBottom && productCodeHasMore.value && !productCodeLoading.value) {
    fetchProductCodes(productCodeSearch.value, productCodePage.value + 1)
  }
}
...
watch(
  () => local.value.discharge_area,
  async (val, oldVal) => {
    if (isInitializing.value) return
    if (val === oldVal) return

    local.value.product_code = ""
    resetPagedLookup(productCodeOptions, productCodePage, productCodeHasMore, productCodeSearch)

    if (val) {
      await fetchProductCodes("", 1)
    }
  }
)
...
function submit() {
  const payload: SamplePayload = {
    ...
    discharge_area: toNumberOrNull(local.value.discharge_area),
    product_code: toNumberOrNull(local.value.product_code),
  ...
  async (v) => {
    ...
    try {
      local.value = {
        ...
        discharge_area: getInitialString("discharge_area"),
        product_code: getInitialString("product_code"),
      ...
      resetPagedLookup(dischargeAreaOptions, dischargeAreaPage, dischargeAreaHasMore, dischargeAreaSearch)
      resetPagedLookup(productCodeOptions, productCodePage, productCodeHasMore, productCodeSearch)

      await fetchSampleTypes("", 1)
      await fetchMaterials("", 1)
      await fetchDischargeAreas("", 1)
      ...
      ensureOptionExists(
        dischargeAreaOptions,
        local.value.discharge_area,
        getInitialLabel("discharge_area_label", "discharge_area_name", "discharge_area_text")
      )
      ...
      if (local.value.discharge_area) {
        await fetchProductCodes("", 1)
        ensureOptionExists(
          productCodeOptions,
          local.value.product_code,
          getInitialLabel("product_code_label", "product_code")
        )
      }
    ...
    <DialogContent class="sm:max-w-5xl max-h-[90vh] overflow-y-auto">
      ...
      <div class="grid gap-4 py-2">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
          ...
          <div class="grid gap-2">
            <label class="text-sm font-medium">Discharge</label>
            <Select v-model="local.discharge_area" :disabled="dischargeAreaLoading || !canMutate">
              <SelectTrigger class="h-9">
                <SelectValue :placeholder="dischargeAreaLoading ? 'Loading...' : 'Select Discharge'" />
              </SelectTrigger>
              <SelectContent class="max-h-80 overflow-auto" @scroll="ondischargeAreaScroll">
                <SelectGroup>
                  <div class="sticky top-0 z-10 bg-background/30 backdrop-blur p-2 border-b">
                    <Input v-model="dischargeAreaSearch" placeholder="Search Discharge..." class="h-8"
                      @input="ondischargeAreaSearch(dischargeAreaSearch)" @keydown.stop @click.stop />
                  </div>
                  ...
                  <div v-if="dischargeAreaLoading" class="p-2 text-sm text-muted-foreground">Loading...</div>
                  <div v-if="!dischargeAreaLoading && dischargeAreaOptions.length === 0"
                    class="p-2 text-sm text-muted-foreground">
                    No results
                  </div>
                </SelectGroup>
            ...
            <p v-if="fieldError('discharge_area')" class="text-sm text-destructive">
              {{ fieldError("discharge_area") }}
            </p>
          </div>

          <div class="grid gap-2">
            <label class="text-sm font-medium">Selling Code</label>
            <Select v-model="local.product_code"
              :disabled="productCodeLoading || !local.discharge_area || !canMutate">
              <SelectTrigger class="h-9">
                <SelectValue
                  :placeholder="!local.discharge_area ? 'Select Discharge first' : (productCodeLoading ? 'Loading...' : 'Select Selling Code')" />
              </SelectTrigger>
              <SelectContent class="max-h-80 overflow-auto" @scroll="onproductCodeScroll">
                <SelectGroup>
                  <div class="sticky top-0 z-10 bg-background/30 backdrop-blur p-2 border-b">
                    <Input v-model="productCodeSearch" placeholder="Search Selling Code..." class="h-8"
                      @input="onproductCodeSearch(productCodeSearch)" @keydown.stop @click.stop />
                  </div>

                  <SelectItem v-for="o in productCodeOptions" :key="o.value" :value="o.value">
                    {{ o.label }}
                  </SelectItem>

                  <div v-if="productCodeLoading" class="p-2 text-sm text-muted-foreground">Loading...</div>
                  <div v-if="!productCodeLoading && productCodeOptions.length === 0"
                    class="p-2 text-sm text-muted-foreground">
                    No results
                  </div>
                </SelectGroup>
            ...
            <p v-if="fieldError('product_code')" class="text-sm text-destructive">
