...
type RowState = {
  ...
  dischargeAreaOptions: LookupOption[]
  dischargeAreaLoading: boolean
  dischargeAreaSearch: string
}
...
function duplicateRow(index: number) {
  ...
  rows.value.splice(index + 1, 0, {
    ...
    dischargeAreaOptions: [...current.dischargeAreaOptions],
...
async function fetchDischargeAreas(row: RowState, q = "") {
  ...
  try {
    ...
    row.dischargeAreaOptions = (res?.results ?? []).map((item: any) =>
      toLookupOption(item, ["value", "id"], ["label", "factory_stock", "name"])
    )
  } finally {
    row.dischargeAreaLoading = false
  }
}
...
const onDischargeAreaSearch = useDebounceFn((row: RowState) => {
  fetchDischargeAreas(row, row.dischargeAreaSearch)
}, 300)
              ...
              <TableCell>
                <Select v-model="row.discharge_area" @update:model-value="() => onDischargeAreaChange(row)">
                  <SelectTrigger class="w-48">
                    <SelectValue :placeholder="row.dischargeAreaLoading ? 'Loading...' : 'Discharge'" />
                  </SelectTrigger>

                  <SelectContent class="max-h-80 overflow-auto">
                    <div class="sticky top-0 z-10 bg-background p-2 border-b">
                      <Input
                        v-model="row.dischargeAreaSearch"
                        placeholder="Search discharge..."
                        class="h-8"
                        @input="onDischargeAreaSearch(row)"
                        @keydown.stop
                        @click.stop
                      />
                    </div>
                    <SelectItem v-for="o in row.dischargeAreaOptions" :key="o.value" :value="o.value">
                      {{ o.label }}
                    </SelectItem>
                  </SelectContent>
                </Select>
