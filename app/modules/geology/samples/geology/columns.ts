...
type ColumnActions = {
  onEdit: (row: GeologyRow) => void
  onDelete: (row: GeologyRow) => void
}
...
export function getGeologyColumns(
  ...
  const cols: ColumnDef<GeologyRow>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => h(DataTableColumnHeader, { column, title: 'Sampling area' }),
      enableSorting: true,
      ...
      ? [
        {
          ...
          cell: ({ row }: { row: { original: GeologyRow } }) =>
