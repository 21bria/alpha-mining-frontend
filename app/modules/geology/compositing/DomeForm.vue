...
import DomeMergeFormDialog, {
  type DomeMergePayload,
} from "@/modules/geology/compositing/components/DomeDialog.vue"

import {
  getDomeMergeColumns,
  type DomeMergeRow,
} from "@/modules/geology/compositing/columns"

import { domeMergeConfig } from "@/modules/geology/compositing/table"
import { domeMergeFilters } from "@/modules/geology/compositing/filters"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<DomeMergeRow>>(
  () => `dome-merge:${JSON.stringify(query.value)}`,
  () => request(domeMergeConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onApply({ search: s, filters }: any) {
  search.value = s ?? ""
  serverFilters.value = { ...(domeMergeConfig.defaultQuery ?? {}), ...(filters ?? {}) }
  page.value = 1
  refresh()
}
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(domeMergeConfig.defaultQuery ?? {}) }
  page.value = 1
  refresh()
}
...
const selected = ref<DomeMergeRow | null>(null)
...
const openView = (row: DomeMergeRow) => {
  formErrors.value = null
  selected.value = row
  mode.value = "edit"
  dialogOpen.value = true
...
const submit = async (payload: DomeMergePayload) => {
  if (!canMutate.value) return
  ...
  try {
    await request(domeMergeConfig.endpoint, {
      method: "POST",
      body: payload,
    })

    notify.success("Dome compositing created")
...
const selectedUndo = ref<DomeMergeRow | null>(null)
const undoLoading = ref(false)

function askUndo(row: DomeMergeRow) {
  if (!canMutate.value) return
...
async function confirmUndo() {
  ...
  try {
    await request(`${domeMergeConfig.endpoint}${selectedUndo.value.id}/undo/`, {
      method: "POST",
      body: {
    ...
    notify.success(
      `Dome compositing "${selectedUndo.value.ref_id ?? selectedUndo.value.id}" undone`
    )
...
const selectedDelete = ref<DomeMergeRow | null>(null)

function remove(row: DomeMergeRow) {
  selectedDelete.value = row
  deleteOpen.value = true
}

async function deleteRows(ids: (string | number)[]) {
  return request("/api/geology/dome-merge/bulk-delete/", {
    method: "POST",
    body: {
      ids,
      hard: true,
...
async function confirmDelete() {
  ...
  try {
    ...
    notify.success(
      `Dome compositing "${selectedDelete.value.ref_id ?? selectedDelete.value.id}" deleted`
    )
...
async function confirmBulkDelete() {
  ...
  try {
    ...
    notify.success(`${bulkDeleteIds.value.length} dome compositing row(s) deleted`)
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    ...
    <DataTableMaster
      ...
      :search="search"
      :loading="pending"
      :filtersSchema="domeMergeFilters"
      :showImport="false"
      :showExport="false"
    ...
    <ConfirmDelete
      ...
      title="Undo Dome Compositing"
      :description="`Are you sure you want to undo '${selectedUndo?.ref_id ?? selectedUndo?.id}'? This will move production back to the original dome.`"
      @confirm="confirmUndo"
    />

    <ConfirmDelete
      v-model:open="deleteOpen"
      title="Delete Dome Compositing"
    ...
    <ConfirmDelete
      ...
      title="Bulk Delete Dome Compositing"
