...
import { getCategoriesColumns, type CategoriesRow } from "@/modules/master/activity/categories/columns"
import { categoriesConfig } from "@/modules/master/activity/categories/table"
import { categoriesFilters } from "@/modules/master/activity/categories/filters"

import CategoriesForm from "@/modules/master/activity/categories/components/CategoriesFormDialog.vue"
...
const serverFilters = ref<Record<string, any>>({ ...(categoriesConfig.defaultQuery ?? {}) })
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<CategoriesRow>>(
  () => `master-activity-categories:${JSON.stringify(query.value)}`,
  () => request(categoriesConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(categoriesConfig.defaultQuery ?? {}) }
  page.value = 1
  refresh()
}
...
const selected = ref<CategoriesRow | null>(null)
const formLoading = ref(false)
const formErrors = ref<Record<string, any> | null>(null)

const deleteOpen = ref(false)
const selectedDelete = ref<CategoriesRow | null>(null)
...
const openEdit = (row: CategoriesRow) => {
  formErrors.value = null
  selected.value = row
  mode.value = "edit"
  dialogOpen.value = true
...
const submit = async (payload: { id?: number; code: string; name?: string | null }) => {
  ...
  try {
    if (mode.value === "create") {
      await request(categoriesConfig.endpoint, { method: "POST", body: payload })
      notify.success(`Categories "${payload.name}" created`)
    } else {
      await request(`${categoriesConfig.endpoint}${payload.id}/`, { method: "PATCH", body: payload })
      notify.success(`Categories "${payload.name}" updated`)
    }
...
const remove = (row: CategoriesRow) => {
...
const confirmDelete = async () => {
  ...
  try {
    await request(`${categoriesConfig.endpoint}${selectedDelete.value.id}/`, { method: "DELETE" })
    notify.success(`Categories "${selectedDelete.value.name}" deleted`)
...
async function confirmBulkDelete() {
  ...
  try {
    await request("/api/masters/activity-categories/bulk-delete/", {
      method: "POST",
