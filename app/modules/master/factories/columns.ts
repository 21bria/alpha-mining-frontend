...
type ColumnActions = {
  onEdit: (row: FactoriesRow) => void
  onDelete: (row: FactoriesRow) => void
}
type CheckState = boolean | "indeterminate"
export function getFactoriesColumns(actions: ColumnActions): ColumnDef<FactoriesRow>[] {
  return [
    {
      id: "select",
      header: ({ table }) =>
    ...
    {
      accessorKey: 'factory_stock',
      header: ({ column }) => h(DataTableColumnHeader, { column, title: 'Name' }),
      enableSorting: true,
      cell: ({ row }) => h('div', { class: 'font-medium' }, row.original.factory_stock),
    },
    {
