...
type MaterialFormState = {
  ...
  name: string
  code: string
  status: string // UI selalu string (Textarea aman)
}
...
watch(
  ...
  (v) => {
    ...
    local.value = {
      ...
      status: props.initial?.status ?? '', // null -> ''
    }
  },
  { immediate: true }
)
...
const submit = () => {
  ...
  const desc = local.value.status.trim()

  emit('submit', {
    id: local.value.id,
    name,
    code,
    status: desc === '' ? null : desc, // kosong -> null
  })
}
    ...
    <DialogContent class="sm:max-w-lg">
      ...
      <div class="grid gap-4 py-2">
        <div class="grid gap-2">
          ...
          <Input v-model="local.code" placeholder="e.g. GC-001" />
      ...
      <DialogFooter>
        ...
        <Button :disabled="loading || !local.name.trim() ||!local.code.trim()" @click="submit">
          {{ loading ? 'Saving...' : 'Save' }}
        </Button>
      </DialogFooter>
    </DialogContent>
