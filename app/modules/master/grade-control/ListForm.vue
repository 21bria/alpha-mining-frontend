...
import { getGeologiesColumns, type GeologiesRow } from "@/modules/master/grade-control/columns"
import { gradeControlConfig } from "@/modules/master/grade-control/table"
import { gradeControlFilters } from "@/modules/master/grade-control/filters"

import GeologiesForm from "@/modules/master/grade-control/components/GeologiesFormDialog.vue"
...
const serverFilters = ref<Record<string, any>>({ ...(gradeControlConfig.defaultQuery ?? {}) })
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<GeologiesRow>>(
  () => `grade-control:${JSON.stringify(query.value)}`,
  () => request(gradeControlConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(gradeControlConfig.defaultQuery ?? {}) }
  page.value = 1
  refresh()
}
...
const selected = ref<GeologiesRow | null>(null)
const formLoading = ref(false)
const formErrors = ref<Record<string, any> | null>(null)

const deleteOpen = ref(false)
const selectedDelete = ref<GeologiesRow | null>(null)
...
const openEdit = (row: GeologiesRow) => {
  formErrors.value = null
  selected.value = row
  mode.value = "edit"
  dialogOpen.value = true
...
const submit = async (payload: { id?: number; name: string; description?: string | null }) => {
  ...
  try {
    if (mode.value === "create") {
      await request(gradeControlConfig.endpoint, { method: "POST", body: payload })
      notify.success(`Grade Control "${payload.name}" created`)
    } else {
      await request(`${gradeControlConfig.endpoint}${payload.id}/`, { method: "PATCH", body: payload })
      notify.success(`Grade Control "${payload.name}" updated`)
    }
...
const confirmDelete = async () => {
  ...
  try {
    await request(`${gradeControlConfig.endpoint}${selectedDelete.value.id}/`, { method: "DELETE" })
...
async function confirmBulkDelete() {
  ...
  try {
    await request("/api/masters/grade-control/bulk-delete/", {
      method: "POST",
...
const columns = getGeologiesColumns({
  onEdit: openEdit,
  onDelete: remove,
})
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    ...
    <DataTableMaster
      ...
      :search="search"
      :loading="pending"
      :filtersSchema="gradeControlFilters"
      :showAdd="true"
      :showImport="false"
      :showExport="true"
    ...
    <GeologiesForm v-model:open="dialogOpen" 
      :mode="mode" 
      :initial="selected" 
      :loading="formLoading"
    ...
    <ImportDialog
      ...
      title="Import Grade Control"
      moduleKey="master.materials"
      importUrl="/api/masters/grade-control/import/"
    ...
    <div v-if="error" class="text-sm text-destructive">
     ...
     <ConfirmDelete v-model:open="deleteOpen" title="Delete Grade Cobtrol"
     ...
     <ConfirmDelete
      ...
      title="Bulk Delete Grade Control"
      :description="`Are you sure you want to delete ${bulkDeleteIds.length} geologies(s)? This action cannot be undone.`"
