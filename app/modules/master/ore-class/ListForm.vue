...
import OreClassFormDialog, {
  type OreClassPayload,
} from "@/modules/master/ore-class/components/OreClassFormDialog.vue"
import {
  getOreClassColumns,
  type OreClassRow,
} from "@/modules/master/ore-class/columns"
import { oreClassConfig } from "@/modules/master/ore-class/table"
import { oreClassFilters } from "@/modules/master/ore-class/filters"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<OreClassRow>>(
  () => `ore-class:${JSON.stringify(query.value)}`,
  () => request(oreClassConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(oreClassConfig.defaultQuery ?? {}) }
  page.value = 1
  refresh()
}
...
const selected = ref<OreClassRow | null>(null)
...
const openEdit = (row: OreClassRow) => {
  if (!canMutate.value) return
  formErrors.value = null
  selected.value = row
  mode.value = "edit"
...
const submit = async (payload: OreClassPayload) => {
  if (!canMutate.value) return
  ...
  try {
    if (mode.value === "create") {
      await request(oreClassConfig.endpoint, { method: "POST", body: payload })
      notify.success(`Ore Class "${payload.ore_class}" created`)
    } else {
      await request(`${oreClassConfig.endpoint}${payload.id}/`, {
        method: "PATCH",
        body: payload,
      })
      notify.success(`Ore Class "${payload.ore_class}" updated`)
    }
...
const selectedDelete = ref<OreClassRow | null>(null)


// delete
function remove(row: OreClassRow) {
  selectedDelete.value = row
  deleteOpen.value = true
}

async function deleteSamples(ids: (string | number)[]) {
  return request("/api/master/ore-class/bulk-delete/", {
    method: "POST",
    body: {
      ids,
      hard: true,
...
async function handleDownloadTemplate() {
  const blob = await request<Blob>("/api/master/ore-class/template/", {
    method: "GET",
  })
  const url = window.URL.createObjectURL(blob)
  const a = document.createElement("a")
  a.href = url
  a.download = "OreClass_import_template.xlsx"
  a.click()
  window.URL.revokeObjectURL(url)
}

async function handleExport() {
  const blob = await request<Blob>("/api/master/ore-class/export/", {
    method: "GET",
  })
  const url = window.URL.createObjectURL(blob)
  const a = document.createElement("a")
  a.href = url
  a.download = "OreClass_export.xlsx"
  a.click()
  window.URL.revokeObjectURL(url)
}
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    ...
    <ImportDialog
      ...
      moduleKey="master.OreClass"
      importUrl="/api/master/ore-class/import/"
