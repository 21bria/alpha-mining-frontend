...
export type DomeTransferPayload = {
  id?: number
...
const emit = defineEmits<{
  ...
  (e: "submit", payload: DomeTransferPayload): void
}>()
...
const title = computed(() =>
  isEdit.value ? "View Transfer Selling" : "Add Transfer Selling"
)

const local = ref<DomeTransferFormState>({
  id: undefined,
  original_dome: null,
  tonnage_primary: "",
  dome_second: null,
...
async function fetchDomeTonnage(
  ...
  try {
    const res = await request<any>("/api/selling/ore/tonnage-by-pile/", {
      method: "GET",
...
function submit() {
  ...
  const payload: DomeTransferPayload = {
    id: local.value.id,
      ...
      <div class="grid gap-4 py-2">
        ...
        <div class="grid gap-2">
          ...
          <p class="text-xs text-muted-foreground">
            {{ primaryLoading ? "Calculating tonnage..." : "Calculated from original dome selling" }}
          </p>
        ...
        <div class="grid gap-2">
          ...
          <p class="text-xs text-muted-foreground">
            {{ secondLoading ? "Calculating tonnage..." : "Calculated from target dome selling" }}
          </p>
