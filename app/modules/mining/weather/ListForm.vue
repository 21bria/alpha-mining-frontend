...
import WeatherForm from "@/modules/mining/weather/components/WeatherFormDialog.vue"
...
import { getweatherColumns, type weatherRow } from "@/modules/mining/weather/columns"
import { weatherConfig } from "@/modules/mining/weather/table"
import { weatherFilters } from "@/modules/mining/weather/filters"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<weatherRow>>(
  () => `mining-weather:${JSON.stringify(query.value)}`,
  () => request(weatherConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(weatherConfig.defaultQuery ?? {}) }
  ordering.value = null
  page.value = 1
  refresh()
}
...
const selected = ref<WeatherDetail | null>(null)
const formLoading = ref(false)
const formErrors = ref<Record<string, any> | null>(null)

// delete state
const deleteOpen = ref(false)
const selectedDelete = ref<weatherRow | null>(null)
...
// edit
async function openEdit(row: weatherRow) {
  formErrors.value = null
  formLoading.value = true

  try {
    const detail = await request<WeatherDetail>(`${weatherConfig.endpoint}${row.id}/`, {
      method: "GET",
    })
...
// submit
async function submit(payload: WeatherPayload) {
  formLoading.value = true
  formErrors.value = null
  ...
  try {
    if (mode.value === "create") {
      for (const item of payload.details) {
        await request(weatherConfig.endpoint, {
          method: "POST",
          body: {
            iup: payload.iup,
            date: payload.date,
      ...
      notify.success("Weather created")
    } else {
      await request(`${weatherConfig.endpoint}${payload.id}/`, {
        method: "PATCH",
        body: payload,
      })
...
async function handleDownloadTemplate() {
  ...
  const data = await request<ArrayBuffer>(
    "/api/mining/daily-weather/download-template/",
    {
      method: "GET",
  ...
  a.href = url
  a.download = "mining_weather_import_template.xlsx"
...
// delete
function remove(row: weatherRow) {
...
async function deleteData(ids: (string | number)[]) {
  return request("/api/mining/daily-weather/bulk-delete/", {
    method: "POST",
    body: {
      ids,
      hard: true,
...
async function confirmDelete() {
  ...
  try {
    ...
    notify.success(`Weather "${selectedDelete.value.date}" deleted`)
...
<template>
  <div class="space-y-4">
    ...
    <ImportDialog 
      ...
      title="Import Daily Weather" 
      moduleKey="mining.weather"
      importUrl="/api/mining/daily-weather/import/" 
    ...
    <DeleteRangeDialog
      ...
      endpoint="/api/mining/daily-weather/delete-range/"
    ...
    <ExportDialog
      ...
      title="Export Daily Weather"
      :filtersSchema="buildExportSchema(currentRole)"
      endpoint="/api/mining/daily-weather/export/"
      method="GET"
      mode="async"
    ...
    <ConfirmDelete v-model:open="deleteOpen" title="Delete Weather"
      :description="`Are you sure you want to delete '${selectedDelete?.date}'? This action cannot be undone.`"
      @confirm="confirmDelete" />

    <ConfirmDelete v-model:open="bulkDeleteOpen" title="Bulk Delete Weather"
      :description="`Are you sure you want to delete ${bulkDeleteIds.length} weather(s)? This action cannot be undone.`"
