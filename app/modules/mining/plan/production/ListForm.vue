...
import FormDialog, {
  type PlanProductionPayload,
} from "@/modules/mining/plan/production/components/FormDialog.vue"
...
import { getplanProductionsColumns, type planProductionsRow } from "~/modules/mining/plan/production/columns"
import { planProductionsConfig } from "~/modules/mining/plan/production/table"
import { planProductionsFilters } from "~/modules/mining/plan/production/filters"
import { buildExportSchema } from "~/modules/mining/plan/production/export"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<planProductionsRow>>(
  () => `plan-production:${JSON.stringify(query.value)}`,
...
// submit
async function submit(payload: PlanProductionPayload) {
  formLoading.value = true
  formErrors.value = null
...
async function handleDownloadTemplate() {
  ...
  const data = await request<ArrayBuffer>(
    "/api/mining/plan-production/download-template/",
    {
      method: "GET",
...
async function deleteData(ids: (string | number)[]) {
  return request("/api/mining/plan-production/bulk-delete/", {
    method: "POST",
    body: {
      ids,
      hard: true,
...
<template>
  <div class="space-y-4">
    ...
    <ImportDialog 
      ...
      title="Import Plan Production" 
      moduleKey="mining.plan_productions"
      importUrl="/api/mining/plan-production/import/" 
    ...
    <DeleteRangeDialog
      ...
      endpoint="/api/mining/plan-production/delete-range/"
    ...
    <ExportDialog
      ...
      endpoint="/api/mining/plan-production/export/"
      method="GET"
      mode="async"
