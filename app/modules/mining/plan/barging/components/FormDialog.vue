...
export type PlanBargingPayload = {
  id?: string
  iup?: number | null
...
const emit = defineEmits<{
  ...
  (e: "submit", payload: PlanBargingPayload): void
}>()
...
const title = computed(() => (props.mode === "create" ? "Add Plan Barging" : "Edit Plan Barging"))
...
const local = ref<PlanBargingFormState>({
  iup: null,
  date_start: "",
  date_end: "",
  category: "BARGING",
...
const selectedIupLabel = computed(() => {
  ...
  return iupOptions.value.find((x) => Number(x.value) === Number(local.value.iup))?.label ?? null
})
...
const selectedVendorLabel = computed(() => {
  if (!local.value.vendors) return null
  return vendorOptions.value.find((x) => String(x.value) === String(local.value.vendors))?.label ?? null
})
...
function getMaterialLabel(value: string | number | null) {
  if (value == null) return null

  return visibleMaterialOptions.value.find(
...
function updateRowMaterial(index: number, value: string) {
  ...
  row.material = value // ini ID
  row.material_label = selected?.label ?? value // ini LIM/SAP name
}
...
function materialKind(row: PlanBargingRow) {
  const raw = `${row.material ?? ""} ${row.material_label ?? ""}`.toLowerCase()

  if (raw.includes("lim")) return "lim"
  if (raw.includes("sap")) return "sap"

  return null
}

const totalLim = computed(() => {
  return local.value.rows.reduce((sum, row) => {
    return materialKind(row) === "lim" ? sum + toNumber(row.tonnage) : sum
  }, 0)
})

const totalSap = computed(() => {
  return local.value.rows.reduce((sum, row) => {
    return materialKind(row) === "sap" ? sum + toNumber(row.tonnage) : sum
  }, 0)
})
...
function submit() {
  ...
  const payload: PlanBargingPayload = {
    id: local.value.id,
    ...
    vendors: local.value.vendors ? String(local.value.vendors) : null,
    lim: totalLim.value,
    sap: totalSap.value,
  }
...
watch(
  ...
  async (v) => {
    ...
    local.value = {
      id: props.initial?.id,
      iup: iupId,
      date_start: props.initial?.date_start ?? props.initial?.date_plan ?? "",
      date_end: props.initial?.date_end ?? props.initial?.date_plan ?? "",
      category: props.initial?.category ?? "BARGING",
      vendors: props.initial?.vendors ?? null,
      rows: [
        ...(Number(props.initial?.lim ?? 0) > 0
          ? [
              {
                material: "LIM",
                material_label: "LIM",
                tonnage: String(props.initial?.lim ?? ""),
              },
            ]
          : []),
        ...(Number(props.initial?.sap ?? 0) > 0
          ? [
              {
                material: "SAP",
                material_label: "SAP",
                tonnage: String(props.initial?.sap ?? ""),
              },
            ]
          : []),
      ],
    ...
    if (!local.value.rows.length) {
      local.value.rows = [{ material: null, material_label: null, tonnage: "" }]
    }
    ...
    if (local.value.vendors && !vendorOptions.value.some((x) => String(x.value) === String(local.value.vendors))) {
      vendorOptions.value = [
        {
          value: local.value.vendors,
          label: props.initial?.vendor_name ?? props.initial?.vendors ?? String(local.value.vendors),
        },
        ...vendorOptions.value,
      ]
    }
      ...
      <div class="grid gap-5 py-2">
        ...
        <div class="grid gap-2">
          ...
          <div class="grid gap-2">
            ...
            <Select
              :model-value="local.vendors ? String(local.vendors) : ''"
              :disabled="vendorLoading || !canMutate"
              @update:model-value="(v) => (local.vendors = v ? String(v) : null)"
            >
              <SelectTrigger class="h-10">
              ...
              <SelectContent class="max-h-80 overflow-auto">
                <SelectGroup>
                  ...
                  <div
                    v-if="local.vendors && selectedVendorLabel"
                    class="border-b bg-background/80 px-2 py-1 text-xs"
                  >
                    Selected:
            ...
            <p v-if="fieldError('vendors')" class="text-sm text-destructive">
              {{ fieldError("vendors") }}
            </p>
          </div>
        </div>
        ...
        <div class="grid gap-3">
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                Add one or more material rows. LIM and SAP totals will be calculated automatically.
              </p>
            </div>
                  ...
                  >
                    ...
                    <SelectContent class="max-h-80 overflow-auto">
                      <SelectGroup>
                        ...
                        <div v-if="!materialLoading && !materialOptions.length" class="p-2 text-sm text-muted-foreground">
                          No results
                        </div>
                      </SelectGroup>
                    </SelectContent>
          ...
          <div class="flex flex-wrap gap-3 text-sm">
            <div class="rounded-lg bg-muted px-3 py-2">
              LIM:
              <span class="font-semibold">{{ totalLim.toLocaleString() }}</span>
            </div>
            <div class="rounded-lg bg-muted px-3 py-2">
              SAP:
              <span class="font-semibold">{{ totalSap.toLocaleString() }}</span>
            </div>
          </div>
        </div>

        <p v-if="fieldError('lim')" class="text-sm text-destructive">
          {{ fieldError("lim") }}
        </p>
        <p v-if="fieldError('sap')" class="text-sm text-destructive">
          {{ fieldError("sap") }}
        </p>
