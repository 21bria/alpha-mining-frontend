...
import RainfallForm from "@/modules/mining/rainfall/components/RainfallFormDialog.vue"
...
import { getRainfallColumns, type RainfallRow } from "@/modules/mining/rainfall/columns"
import { rainfallConfig } from "@/modules/mining/rainfall/table"
import { rainfallFilters } from "@/modules/mining/rainfall/filters"
import { buildExportSchema } from "@/modules/mining/rainfall/export"
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<RainfallRow>>(
  () => `mining-rainfall:${JSON.stringify(query.value)}`,
  () => request(rainfallConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(rainfallConfig.defaultQuery ?? {}) }
  ordering.value = null
  page.value = 1
  refresh()
}
...
const selected = ref<RainfallDetail | null>(null)
const formLoading = ref(false)
const formErrors = ref<Record<string, any> | null>(null)

// delete state
const deleteOpen = ref(false)
const selectedDelete = ref<RainfallRow | null>(null)
...
// edit
async function openEdit(row: RainfallRow) {
  formErrors.value = null
  formLoading.value = true

  try {
    const detail = await request<RainfallDetail>(`${rainfallConfig.endpoint}${row.id}/`, {
      method: "GET",
    })
...
// submit
async function submit(payload: RainfallPayload) {
  formLoading.value = true
  formErrors.value = null

  try {
    if (mode.value === "create") {
      await request(rainfallConfig.endpoint, {
        method: "POST",
        body: payload,
      })
      notify.success("Rainfall created")
    } else {
      await request(`${rainfallConfig.endpoint}${payload.id}/`, {
        method: "PATCH",
        body: payload,
      })
      notify.success("Rainfall updated")
    }
...
async function handleDownloadTemplate() {
  ...
  const data = await request<ArrayBuffer>(
    "/api/mining/daily-rainfall/download-template/",
    {
      method: "GET",
  ...
  a.href = url
  a.download = "mining_rainfall_import_template.xlsx"
...
// delete
function remove(row: RainfallRow) {
  selectedDelete.value = row
  deleteOpen.value = true
}

async function deleteData(ids: (string | number)[]) {
  return request("/api/mining/daily-rainfall/bulk-delete/", {
    method: "POST",
    body: {
      ids,
      hard: true,
...
async function confirmDelete() {
  ...
  try {
    ...
    notify.success(`Rainfall "${selectedDelete.value.date}" deleted`)
...
<template>
  <div class="space-y-4">
    ...
    <ImportDialog 
      ...
      title="Import Daily Rainfall" 
      moduleKey="mining.rainfall"
      importUrl="/api/mining/daily-rainfall/import/" 
    ...
    <DeleteRangeDialog
      ...
      endpoint="/api/mining/daily-rainfall/delete-range/"
    ...
    <ExportDialog
      ...
      title="Export Daily Rainfall"
      :filtersSchema="buildExportSchema(currentRole)"
      endpoint="/api/mining/daily-rainfall/export/"
      method="GET"
      mode="async"
    ...
    <ConfirmDelete v-model:open="deleteOpen" title="Delete Rainfall"
      :description="`Are you sure you want to delete '${selectedDelete?.date}'? This action cannot be undone.`"
      @confirm="confirmDelete" />

    <ConfirmDelete v-model:open="bulkDeleteOpen" title="Bulk Delete Rainfall"
      :description="`Are you sure you want to delete ${bulkDeleteIds.length} rainfall(s)? This action cannot be undone.`"
