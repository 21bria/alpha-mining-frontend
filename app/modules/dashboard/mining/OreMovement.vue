...
const defaultChartType = computed(() => props.filterType === 'monthly' ? 'area' : 'bar')

async function fetchChartData() {
  try {
     const filterType = props.filterType

    const endpoint = '/api/analytics/raw/mining/chart-ore/'
    ...
    grandTotal.value = data.grand_total ?? { lim: 0, sap: 0, total: 0, avg: 0 }
    ...
    if (props.filterType === 'monthly') {
      chartData.value = {
        ...
        series: [
          { name: 'Actual', type: 'area', yAxisIndex: 0, data: data.total_tonnage ?? [] },
          { name: 'Plan', type: 'line', yAxisIndex: 1, data: data.total_plan ?? [] }
        ]
      }
    } else {
      chartData.value = {
