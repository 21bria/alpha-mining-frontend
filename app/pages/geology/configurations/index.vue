...
import { geologyConfigItems } from "~/constants/menus/geology-config-items"
...
const search = ref("")
const activeCategory = ref("All Config")
const openGroups = ref<string[]>([])

const categories = ["All Config", "Master", "Adjustment", "Status"]
...
const permittedMasterItems = computed(() => {
  return filterNavMenuItems(geologyConfigItems as any, userPerms.value)
})
...
const safeMasterItems = computed(() => {
  return permittedMasterItems.value.map((item: any) => {
    const children =
      "children" in item
...
const filteredItems = computed(() => {
  ...
  let items = safeMasterItems.value

  if (activeCategory.value !== "All Config") {
    ...
    <div class="p-4">
      ...
      <div class="flex flex-col gap-4 border-b pb-4">
        <div class="flex items-start justify-between gap-4">
          <div>
            <h2 class="text-lg font-semibold tracking-tight">
              Geology Configurations
            </h2>
            <p class="text-sm text-muted-foreground">
              Browse and access all geology configuration modules
            </p>
          </div>
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div class="relative w-full lg:w-[380px]">
            ...
            <input
              v-model="search"
              placeholder="Search geology configurations"
      ...
      <!-- Content -->
        <div class="max-h-[62vh] overflow-y-auto py-7 pr-2">
             ...
             <div
                ...
                >
                    ...
                    <p v-if="!isGroupOpen(item.title)" class="mt-1 text-sm text-muted-foreground">
                        {{ item.children.length }} master data options
                    </p>
                    </div>
                </button>
          ...
          >
            No geology configurations found.
          </div>
        </div>
    </div>
  </div>
