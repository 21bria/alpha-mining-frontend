...
import LoadingFormDialog, { type SourceMinesLoadingPayload } from "@/modules/master/loading/components/LoadingFormDialog.vue"
import { getSourceMineLoadingColumns, type SourceMineLoadingRow } from "@/modules/master/loading/columns"
import { mineLoadingConfig } from "@/modules/master/loading/table"
import { mineLoadingFilters } from "~/modules/master/loading/filters"
...
const rows = ref<SourceMineLoadingRow[]>([])
const total = ref(0)
...
const serverFilters = ref<Record<string, any>>({ ...(mineLoadingConfig.defaultQuery ?? {}) })
...
const { data, pending, error, refresh } = await useAsyncData<ApiList<SourceMineLoadingRow>>(
  () => `Loading Point:${JSON.stringify(query.value)}`,
  () => request(mineLoadingConfig.endpoint, { method: "GET", query: query.value }),
  { server: false }
)
...
function onReset() {
  search.value = ""
  serverFilters.value = { ...(mineLoadingConfig.defaultQuery ?? {}) }
  page.value = 1
  refresh()
}
...
const selected = ref<SourceMineLoadingRow | null>(null)
...
const openEdit = (row: SourceMineLoadingRow) => {
  if (!canMutate.value) return
  formErrors.value = null
  selected.value = row
  mode.value = "edit"
...
const submit = async (payload: SourceMinesLoadingPayload) => {
  if (!canMutate.value) return
  ...
  try {
    if (mode.value === "create") {
      await request(mineLoadingConfig.endpoint, { method: "POST", body: payload })
      notify.success(`Loading Point "${payload.loading_point}" created`)
    } else {
      await request(`${mineLoadingConfig.endpoint}${payload.id}/`, { method: "PATCH", body: payload })
      notify.success(`Loading Point "${payload.loading_point}" updated`)
    }
...
const selectedDelete = ref<SourceMineLoadingRow | null>(null)

const remove = (row: SourceMineLoadingRow) => {
  if (!canMutate.value) return
...
const confirmDelete = async () => {
  ...
  try {
    await request(`${mineLoadingConfig.endpoint}${selectedDelete.value.id}/`, { method: "DELETE" })
    notify.success(`Loading Point "${selectedDelete.value.loading_point}" deleted`)
...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    <div class="flex flex-wrap items-end justify-between gap-2">
      <div>
        <h2 class="text-2xl font-bold tracking-tight">Loading Points</h2>
        <p class="text-muted-foreground">List of Loading Points</p>
      </div>
    </div>
    ...
    <DataTableMaster
      ...
      :search="search"
      :loading="pending"
      :filtersSchema="mineLoadingFilters"
      :showSyncData="false"
    ...
    <ConfirmDelete
      ...
      :description="`Are you sure you want to delete '${selectedDelete?.loading_point}'? This action cannot be undone.`"
